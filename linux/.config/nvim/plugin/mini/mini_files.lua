local center_layout = function(ev)
  ...
  win_config.row = math.ceil(0.5 * (vim.o.lines - win_config.height)) - 2
...
local function setup_mini_files()
  ...
  local function on_mini(mini_files_event, cb)
    on("User", augroup, { pattern = "MiniFiles" .. mini_files_event }, cb)
  end
  ...
  on_mini("WindowOpen", function(ev)
  ...
  on_mini("WindowUpdate", current_layout)
  ...
  local map_split = function(buf_id, lhs, direction)
    ...
    map(lhs, rhs, { buffer = buf_id, desc = desc })
  end
...
pack.gen_loop_loader(function() setup_mini_files() end)("mini.files")
